...
protected:
  ...
  struct stat CallLstatWithImpl(std::function<void(struct FUSE_STAT*)> implementation);
  ...
  struct stat CallFileLstatWithImpl(std::function<void(struct FUSE_STAT*)> implementation);
  struct stat CallDirLstatWithImpl(std::function<void(struct FUSE_STAT*)> implementation);

private:

  struct stat CallLstatWithModeAndImpl(mode_t mode, std::function<void(struct FUSE_STAT*)> implementation);
};
