...
public:
  ...
  int getattr(const boost::filesystem::path &path, struct FUSE_STAT *stbuf);
  int fgetattr(const boost::filesystem::path &path, struct FUSE_STAT *stbuf, fuse_file_info *fileinfo);
